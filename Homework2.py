...
# задача не проходит один тест, но логика верна, грубых ошибок нет, поэтом рекомендую посмотреть ответы
...
# Рабочее решение, все тесты проходит. Для более python решения, см ответы
def triangle(a, b, c):
...
def longest_word(sentence):
    ...
    if sentence=='' or not type(sentence)==str: #убираем результаты, где входные значения не строка или строка пустая
        ...
        # очень интересное решение через translate
        k=sentence.translate(str.maketrans('','',string.punctuation)) #убираем пунктуацию в предложении
        longest_word='' #создаем пустую строку
        # тоже хорошее решение, которое закрывает условие "вернуть последнее длинное слово"
        # для более pythonic - см ответы
...
# Эти символы, в идеале, стоит удалить =)
#%%
